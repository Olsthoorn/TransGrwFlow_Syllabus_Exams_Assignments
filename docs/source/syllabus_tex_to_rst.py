#! python3
'''Convert sysllabus tex file tot rst file.
...
But after the rst files have been generated they need to be filtered, which is also done in this file.

The filterin includes:
    Duplicate figure numbers have to be removeed.
...
(There can only be one label in a multiline math formula block.)
...
Ths virtual enviromment has been stocked by the required packages and the readthedocs theme.
...
def avoidDupNames(infile, outfile):
    """Filter out duplicate figure and table names, and if found add a range number.
    
    Multiple equivalent labels originate from figure floats with more than one image (.png) in
    them. The resulting rst file then yields multiple figures with the same label. This is resolved by
    appending an underscore and a follow-up number (2, 3, 4) ect to these labels.
    ...
    Args:
        ...
        outfile (.tmp): file generated by this function for further processing in project directory.
...
def clean_labels_and_references(infile, outfile):
    """Replace labels for figs, tables, and equations to make them seen by sphinx.
    
    To make references in Sphinx work, we have to convert the ones obtained from
    
    LyX --> Latex --> pandoc --> rst file
    
    There were a lot of problems, see comments in the code below, next to
    some settings in the conf.py file.
    ...
    which was already processed. It is stored in the stack, which is then written out upon encountering
    the end of the block, while inserting the new label line as second line of the block.
    
    Args:
        infile (.tmp): temporary rst file in the project directory without duplicated labels (generated by avoidLabels)
...
if __name__ == '__main__':
    ...
    for d in [syllabus, root, docs, source, build]:
        assert os.path.isdir(d), FileNotFoundError(d)
    
    os.chdir(source)
    logging.warning("Source dir: {}".format(os.getcwd()))
    ...
    if False:
        # Make the html documentation in the build directory
        # There must be no errors in previous steps to run this one automatically.
